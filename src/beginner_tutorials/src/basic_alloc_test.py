...
def do_tasks():
    ...
    while i < (len(tasks.data)):
        ...
        global task_index
        print(name, "length of task list", len(tasks.data))
        ...
        skip_task = False
        for n in range(len(turtlebot_names)):
            for m in range(1, len(completed_tasks.data[n])):
        ...
        max_task_duration = distance * 10 + 15
